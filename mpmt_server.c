...
extern void * http_file_thread (void *_id);
void child_main(int sockfd, int pipefd)
{
	 ...
	 build_JVM();
	//		 for (thd_id2 = 2; thd_id2 < 100; thd_id2++)
     ...
     ret = ci_thread_create(&thread, (void *(*)(void *)) test_ft,
                            &thd_id);
     
	 for(thd_id2=2;thd_id2<1000;thd_id2++)
     ret = ci_thread_create(&http_session_thread[thd_id2], (void *(*)(void *)) http_file_thread,
                            &thd_id2);
     /*set srand for child......*/
