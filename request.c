...
http_file_thread (void *_id)
{
    ...
    while (1)
    {
        ...
        if (sbuff_getdata (http_buffer_id, sshdr) > 0)
        {			//等待信号触发，如果触发成功，执行如下操作
            log_printf(ZLOG_LEVEL_DEBUG, "\n\n\n\nhttp file process Sbuff_getdata OK ! ! ! ! \n\n\n\n");
#if 1
            HttpSession sp_session;
            memset(&sp_session,0x0,sizeof(HttpSession));
            ...
            http_session = (dlp_http_post_head* )sp_session.http;
            //http_session->current_thread_id = pthread_self();
            log_printf(ZLOG_LEVEL_DEBUG, "\nhttp file http_session->new_name = %s\n",http_session->new_name);
            log_printf(ZLOG_LEVEL_DEBUG, "\nhttp file http_session->boundary = %s\n",http_session->boundary);
...
void request_stats_init()
{
    ...
    for(thd_id2=2;thd_id2<1000;thd_id2++)
        ret =pthread_create(&http_session_thread[0], (void *(*)(void *)) http_file_thread,&thd_id2);
#endif
...
#if 0
ci_request_t *testresquest(ci_connection_t * connection)
{
    ci_request_t *req;
    int access;
    int len;
...
/*Here we want to read in small blocks icap header because in most cases
...
static int ci_read_icap_header(ci_request_t * req, ci_headers_list_t * h, int timeout)
{
    ...
    int wait_status = 0;
    char *buf_end; //defined here.
    int dataPrefetch = 0;   

    buf_end = h->buf; //指针 copy
    readed = 0;
    bytes = 0;

    //?
    if (PIPELINING && req->pstrblock_read && req->pstrblock_read_len > 0) {
        if ((request_status = icap_header_check_realloc(&(h->buf), &(h->bufsize), req->pstrblock_read_len, ICAP_HEADER_READSIZE)) != EC_100)
    ...
    do {
        ...
        if ((request_status = icap_header_check_realloc(&(h->buf), &(h->bufsize), readed, ICAP_HEADER_READSIZE)) != EC_100)
            break;
    ...
    //h->bufused 是ICAP头的长度 只包含一个/r/n
    h->bufused = buf_end - h->buf;     /* -1 ; */
   
    //req->pstrblock_read 指向HTTP头，略过了ICAP头
    req->pstrblock_read = buf_end + 2; /*after the \r\n\r\n. We keep the first \r\n and the other dropped.... */
    
    //req->pstrblock_read_len 下次应该读的字节数
...
static int read_encaps_header(ci_request_t * req, ci_headers_list_t * h, int size)
{
    ...
    //req->pstrblock_read 指向HTTP头开始位置，略过了ICAP头
    ...
    remains = size - readed; //remain bytes
    while (remains > 0) {
    ...
    req->http_bytes_in += size;  //first here?
...
static int parse_request(ci_request_t * req, char *buf)
{
    ...
    len = strcspn(start, "/ ");
    //ci_debug_printf(9, "===========================len is %d\n", len);
    end = start + len;

    //servnamelen是icap server的ip地址，和端口的字符串
    ...
    if (*end == '/') { /*we are expecting service name*/
        ...
        len = (len < MAX_SERVICE_NAME ? len : MAX_SERVICE_NAME);
        if (len) {
        ...
        if (*end == '?') {     /*args */
            ...
            if ((end = strchr(start, ' ')) != NULL) {
                ...
                len = (len < (MAX_SERVICE_ARGS - args_len) ? len : (MAX_SERVICE_ARGS - args_len));
    ...
    if (req->service[0] != '\0') {
        //为什么可以找到 ???????
        /* ci_debug_printf(9, "========================================================have service\n"); */
...
static int check_request(ci_request_t *req)
{
    ...
    //"Encapsulated: " process_encapsulated 里边初始化的
...
static int parse_header(ci_request_t * req)
{
    ...
    for (i = 1; i < h->used && request_status == EC_100; i++) {
        if (strncasecmp("Preview:", h->headers[i], 8) == 0) {
            /* ci_debug_printf(9, "\033[0;35m=============Preview, i is %d============\n\033[0m", i); */
            val = h->headers[i] + 8;
            for (;isspace(*val) && *val != '\0'; ++val);
            errno = 0;
            result = strtol(val, NULL, 10); //读取为10进制数
...
static int parse_encaps_headers(ci_request_t * req)
{
    ...
    for (i = 0; (e = req->entities[i]) != NULL; i++) {
        ...
        //如果是ICAP_REQ, e->entity 就是h->ci_headers_list_t
...
static int get_send_body(ci_request_t * req, int parse_only)
{
    ...
    do {
        ...
        if (action) {
            ...
            // );
            if ((ret = wait_for_data(req->connection->fd, TIMEOUT, action)) < 0)
                break;
...
static int do_request(ci_request_t * req)
{
    ...
    //正常返回EC_100 after check_request 
    if (res != EC_100) {
    ...
    //这里开始传递参数
    srv_xdata = service_data(req->current_service_mod);  //service_data-->  return &(service_extra_data_list[i]);
    ...
    //这里将req发送给service: echo_init_request_data
